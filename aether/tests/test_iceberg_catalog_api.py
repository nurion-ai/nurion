...
def test_list_namespaces(catalog):
    ...
    assert ("default",) in namespaces or any(
        ns == ("default",) or ns == ["default"] for ns in namespaces
    )
...
def test_create_namespace(catalog):
    ...
    properties = {
        ...
        "location": f"file:///tmp/warehouse/test_db_{unique_id}",
    }
    ...
    assert namespace in all_namespaces or any(
        ns == namespace or list(ns) == list(namespace) for ns in all_namespaces
    )
...
def test_create_table(catalog, warehouse_path):
    ...
    try:
        ...
        if isinstance(name_result, tuple):
            expected_msg = f"Expected table name '{table_name}', got '{name_result[1]}'"
            assert name_result[1] == table_name, f"{expected_msg} in tuple {name_result}"
        else:
...
def test_update_namespace_properties(catalog):
    ...
    catalog.update_namespace_properties(
        namespace,
        removals=[],
        updates={"description": "Updated description", "custom_prop": "value"},
    )
...
def test_complete_workflow(catalog, warehouse_path):
    ...
    if isinstance(loaded_name_result, tuple):
        expected_msg = f"Expected table name '{table_name}', got '{loaded_name_result[1]}'"
        assert loaded_name_result[1] == table_name, f"{expected_msg} in tuple {loaded_name_result}"
    else:
...
def test_multiple_namespaces_and_tables(catalog, warehouse_path):
    ...
    all_namespaces = catalog.list_namespaces()
    for ns in namespaces:
        assert ns in all_namespaces or any(
            list(ns) == list(existing) for existing in all_namespaces
        )
