...
def test_legacy_spark_on_fractional_cpu():
    cluster = Cluster(
        initialize_head=True, connect=True, head_node_args={"num_cpus": 2}
    )

    spark = raydp.init_spark(
        app_name="test_cpu_fraction",
        num_executors=1,
        executor_cores=3,
...
def test_spark_executor_node_affinity():
    ...
    spark = raydp.init_spark(
        app_name="test_executor_node_affinity",
        num_executors=1,
        executor_cores=2,
        executor_memory="500M",
...
def test_ray_dataset_roundtrip(spark_on_ray_2_executors):
    ...
    assert values == rows
    df = raydp.spark.dataset.ray_dataset_to_spark_dataframe(
        spark, ds.schema(), ds.get_internal_block_refs()
    )
...
def test_ray_dataset_to_spark(spark_on_ray_2_executors):
    ...
    df2 = raydp.spark.dataset.ray_dataset_to_spark_dataframe(
        spark, ds2.schema(), ds2.get_internal_block_refs()
    )
...
def test_placement_group(ray_cluster):
    for pg_strategy in ["PACK", "STRICT_PACK", "SPREAD", "STRICT_SPREAD"]:
        spark = raydp.init_spark(
            f"test_strategy_{pg_strategy}_1",
            1,
            1,
            "500M",
        ...
        pg = ray.util.placement_group(
            [{"CPU": 1, "memory": utils.parse_memory_size("500M")}],
            strategy=pg_strategy,
        )
        ray.get(pg.ready())
        spark = raydp.init_spark(
            f"test_bundle_{pg_strategy}_2",
            1,
            1,
            "500M",
            placement_group=pg,
        ...
        spark = raydp.init_spark(
            f"test_bundle_{pg_strategy}_3", 1, 1, "500M", placement_group=pg
        )
        result = spark.range(0, 10, numPartitions=10).count()
        assert result == 10
        raydp.stop_spark()
...
def test_reconstruction():
    ...
    head = cluster.add_node(
        num_cpus=2, include_dashboard=False, enable_object_reconstruction=True
    )
    ...
    node_to_kill = cluster.add_node(
        num_cpus=1, include_dashboard=False, object_store_memory=10**8
    )
