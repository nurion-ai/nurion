...
class TestMapBatchesOperator:
    ...
    def test_map_batches_basic(self):
        ...
        def process_batch(records):
            ...
            return [
                Record(key=r.key, value={'value': r.value['value'] * 2})
                for r in records
            ]
    ...
    def test_map_batches_skip_on_error(self):
        ...
        operator = MapBatchesOperator({
            'map_batches_fn': failing_fn,
            'skip_on_error': True
        })
        context = OperatorContext('task1', 'stage1', 'worker1')
        operator.open(context)
        ...
        batch = Batch(
            records=[Record(key='1', value={'data': 'test'})],
            batch_id='batch1'
        )
    ...
    def test_map_batches_aggregation(self):
        ...
        def aggregate_batch(records):
            ...
            return [Record(
                key='aggregated',
                value={'total': total, 'count': len(records), 'avg': avg}
            )]
...
class TestFilterOperator:
    ...
    def test_filter_with_complex_condition(self):
        ...
        def is_valid(record):
            return (
                record.get('score', 0) > 0.5 and
                record.get('count', 0) > 10
            )
