...
class TestMemoryClientGC:
    ...
    async def test_truncate_before_removes_old_records(self):
        ...
        # Produce 10 messages
        for i in range(10):
            await client.produce(topic, f"msg-{i}".encode())

        # Truncate before offset 5
        deleted = await client.truncate_before(topic, 5)
        assert deleted == 5, f"Expected 5 deleted, got {deleted}"

        # Fetch should only return records 5-9
        records = await client.fetch(topic, offset=0, max_records=20)
    ...
    async def test_truncate_nonexistent_topic(self):
        ...
        deleted = await client.truncate_before("nonexistent", 100)
        assert deleted == 0
    ...
    async def test_get_min_committed_offset_single_group(self):
        ...
        await client.commit_offset("group1", topic, 50)

        min_offset = await client.get_min_committed_offset(topic)
    ...
    async def test_get_min_committed_offset_multiple_groups(self):
        ...
        await client.commit_offset("group1", topic, 100)
        await client.commit_offset("group2", topic, 50)  # Slowest
        await client.commit_offset("group3", topic, 75)
    ...
    async def test_gc_workflow(self):
        ...
        await client.commit_offset("fast-consumer", topic, 80)
        await client.commit_offset("slow-consumer", topic, 30)
        ...
        deleted = await client.truncate_before(topic, min_offset)
        assert deleted == 30

        # Slow consumer can still read its next record
        records = await client.fetch(topic, offset=30, max_records=1)
        assert len(records) == 1
        assert records[0].offset == 30

        # Fast consumer can continue from where it was
        records = await client.fetch(topic, offset=80, max_records=100)
