...
from solstice.core.stage_master import StageMaster, StageConfig, QueueType, QueueEndpoint, StageStatus, QueueMessage
...
class TestBackpressureDetection:
    ...
    @pytest.mark.asyncio
    async def test_backpressure_activated_on_high_lag(self, payload_store, tansu_backend, ray_cluster):
    ...
    @pytest.mark.asyncio
    async def test_backpressure_not_activated_on_low_lag(self, payload_store, tansu_backend, ray_cluster):
    ...
    @pytest.mark.asyncio
    async def test_backpressure_activated_on_high_queue_size(self, payload_store, tansu_backend, ray_cluster):
...
class TestSourceRateControl:
    ...
    async def test_pause_when_downstream_has_backpressure(self, payload_store, ray_cluster):
        ...
        downstream_port = 10000 + random.randint(0, 9999)
        downstream_config = StageConfig(
            queue_type=QueueType.TANSU,
            max_workers=2,
            tansu_storage_url="memory://tansu/",
            tansu_port=downstream_port,
        )
