...
class TestMemoryBackendExactlyOnce:
    ...
    async def test_resume_after_crash(self, memory_backend: MemoryBackend):
        ...
        await memory_backend.fetch(input_topic, offset=0, max_records=5)
    ...
    @pytest.mark.asyncio
    async def test_crash_before_commit_causes_reprocess(self, memory_backend: MemoryBackend):
        ...
        offset = 0
        for _ in range(3):
            records = await memory_backend.fetch(input_topic, offset=offset, max_records=1)
            if records:
                await memory_backend.produce(output_topic, b"processed-" + records[0].value)
        ...
        restart_offset = await memory_backend.get_committed_offset(group, input_topic) or 0
        ...
        output_records = await memory_backend.fetch(output_topic, offset=0, max_records=20)
        assert len(output_records) == 8

    @pytest.mark.asyncio
    async def test_idempotent_processing_achieves_exactly_once(self, memory_backend: MemoryBackend):
