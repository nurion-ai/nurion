...
import org.apache.logging.log4j.core.config.{Configurator, DefaultConfiguration}
...
private[spark] class SparkSubmit extends Logging {
  ...
  def doSubmit(args: Array[String]): Unit = {
    ...
    // builder.setConfigurationName("raydpConfig")
    //   .setStatusLevel(Level.INFO)
    // val appenderBuilder = builder.newAppender("Console", "CONSOLE").addAttribute("target", ConsoleAppender.Target.SYSTEM_OUT)
    // val custom = new java.util.HashMap[String, String]()
    // custom.put("component", "raydp-driver")
    // custom.put("job_id", System.getProperty("ray.job.id", ""))
    // appenderBuilder.add(builder
    //   .newLayout("PatternLayout")
    //   .addAttribute("pattern", s"%d{yyyy-MM-dd HH:mm:ss.SSS} %p %pid %tid - %c{1}:%L ${new Gson().toJson(custom)} [%t] : %m%n"))
    // builder.add(appenderBuilder)
    //
    // val rootLogger = builder.newRootLogger(Level.INFO)
    // rootLogger.add(builder.newAppenderRef("Console"))
    // builder.add(rootLogger)
    // builder.add(builder.newLogger("org.apache.hadoop.metrics2", Level.ERROR))
    //
    // appArgs.sparkProperties.filter {
    //   case (k, _) => k.toLowerCase.startsWith(RayDPConfigs.SPARK_LOGGER_PREFIX)
    // }.foreach { case (k, v) =>
    //   val loggerName = k.substring(RayDPConfigs.SPARK_LOGGER_PREFIX.length)
    //   builder.add(builder.newLogger(loggerName, Level.valueOf(v)))
    // }
