...
private[spark] class PythonWorkerFactory(pythonExec: String, envVars: Map[String, String])
  ...
  @GuardedBy("self")
  private val simpleWorkers = new mutable.WeakHashMap[Socket, PyActorHandle]()
  ...
  def create(): (Socket, Option[Int]) = {
    log.info(s"create worker for executor-${SparkEnv.get.executorId}")
    if (useDaemon) {
      self.synchronized {
        if (idleWorkers.nonEmpty) {
          val worker = idleWorkers.dequeue()
          logInfo(s"got idle worker ${worker}, port => ${worker.getPort}")
          return (worker, daemonWorkers.get(worker))
        }
      }
      logInfo("try to create new daemon worker")
      createThroughDaemon()
    } else {
      self.synchronized {
        if (idleWorkers.nonEmpty) {
          val worker = idleWorkers.dequeue()
          val port = worker.getPort
          val (socket, pid) = createSocket(port)
          logInfo(s"got idle worker ${worker}, ${pid}, port => ${port}")
          return (socket, Some(pid))
        }
      }
      logInfo("try to create new simple worker")
   ...
   */
  private def createSocket(port: Int): (Socket, Int) = {
    val host = "127.0.0.1"
    ...
    try {
      ...
      try {
        self.synchronized {
          ...
          val (socket, pid) = createSocket(port)
          simpleWorkers.put(socket, handle)
          (socket, Some(pid))
        }
      } catch {
        case e: Exception =>
  ...
  private def stopDaemon(): Unit = {
    self.synchronized {
      if (useDaemon) {
        ...
        simpleWorkers.mapValues(_.kill())
      }
    }
  }
  ...
  def stopWorker(worker: Socket): Unit = {
    self.synchronized {
      if (useDaemon) {
        ...
        simpleWorkers.get(worker).foreach(_.kill())
      }
    }
    worker.close()
  }
  ...
  def releaseWorker(worker: Socket): Unit = {
    logInfo(s"release worker ${worker}")
    if (useDaemon) {
      self.synchronized {
        lastActivityNs = System.nanoTime()
