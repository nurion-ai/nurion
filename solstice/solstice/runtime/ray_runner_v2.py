"""Ray runtime v2 for executing Solstice jobs with queue-based architecture.

This runner uses the new stage_master_v2 architecture:
- Workers pull directly from upstream queues
- Masters only manage their output queue
- Offset-based recovery (no complex checkpointing)
"""
...
class RayJobRunnerV2:
    """Job runner using v2 queue-based architecture.
    
    Key differences from v1:
    - Uses StageMasterV2 (simplified, output-queue only)
    - Workers pull from upstream queues (not master-to-master)
    ...
    def __init__(
        ...
        self._masters: Dict[str, StageMasterV2] = {}
