...
class MetaService:
    ...
    def add_stage(
        ...
        self.logger.info(
            f"Added stage {stage_id} with {len(upstream_stages)} upstream stages"
        )
    ...
    def propagate_backpressure(self, from_stage: str, slow_down_factor: float) -> None:
        ...
        self.logger.info(
            f"Propagating backpressure from {from_stage} "
            f"with factor {slow_down_factor}"
        )
        ...
        for stage_id in upstream:
            self.logger.debug(
                f"Backpressure signal to stage {stage_id}: "
                f"slow down by {slow_down_factor}"
            )
    ...
    def trigger_global_checkpoint(self) -> Optional[str]:
        ...
        try:
            checkpoint_id = ray.get(
                self.global_state_master.trigger_global_checkpoint.remote(),
                timeout=60
            )
