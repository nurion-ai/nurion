...
class MapOperator(Operator):
    ...
    def process_split(self, split: Split, batch: Optional[SplitPayload] = None) -> Optional[SplitPayload]:
    ...
    def process_split(self, split: Split, batch: Optional[SplitPayload] = None) -> Optional[SplitPayload]:
        ...
        try:
            ...
            if len(new_data) != len(batch):
                raise ValueError("map_batches_fn must return the same number of records as the input batch")
            return batch.with_new_data(new_data, split_id=f"{split.split_id}_{self.worker_id}")
        except Exception as e:
